...
export default function PlantJournal() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        setEntries(docs.filter((d: any) => d.userId === u.uid));
      },
      (error) => {
        handleFirestoreError(error, OperationType.LIST, "plant_journals");
      },
    );
    return () => unsubscribe();
  }, [u]);
  ...
  const openForm = (entry?: any) => {
    if (entry) {
      ...
      setFormParams({
        ...
        type: entry.type || "Balcony Garden",
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        await updateDoc(doc(db, "plant_journals", editingId), {
        ...
        await addDoc(collection(db, "plant_journals"), {
          userId: u.uid,
          title: formParams.title,
          date: formParams.date,
          content: formParams.content,
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteDoc(doc(db, "plant_journals", id));
    } catch (e) {
      handleFirestoreError(e, OperationType.DELETE, `plant_journals/${id}`);
    }
  };
    ...
    <div className="p-4 lg:p-6 space-y-8 animate-in fade-in duration-700 max-w-5xl mx-auto min-h-screen">
      ...
      {!u ? (
        <div className="text-center py-24 bg-surface border border-outline/20 rounded-2xl">
          <Sprout className="w-16 h-16 mx-auto mb-6 opacity-30 text-[#5D7964]" />
          <p className="text-xl font-display font-bold text-on-surface mb-4">
            로그인하여 식물 일지를 작성해보세요.
          </p>
          <button
            onClick={signIn}
            className="flex items-center gap-2 mx-auto bg-[#5D7964] text-white px-8 py-3 rounded-lg font-bold hover:shadow-lg transition-all"
          >
