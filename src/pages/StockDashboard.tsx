...
const MOCK_STOCKS: Record<string, CoinInfo> = {
  samsung: {
    name: "삼성전자",
    symbol: "005930",
...
export default function StockDashboard() {
  ...
  const [buyPrices, setBuyPrices] = useState<Record<string, number>>(() => {
    try {
      const saved = localStorage.getItem("stock_buy_prices_krw");
      return saved ? JSON.parse(saved) : { samsung: 71000, hynix: 156000, hyundai: 230000 };
    } catch {
      return { samsung: 71000, hynix: 156000, hyundai: 230000 };
    }
  });

  const [quantities, setQuantities] = useState<Record<string, number>>(() => {
    try {
      const saved = localStorage.getItem("stock_quantities");
      return saved ? JSON.parse(saved) : { samsung: 100, hynix: 50, hyundai: 30 };
    } catch {
      return { samsung: 100, hynix: 50, hyundai: 30 };
    }
  });
  ...
  const currentBuyPrice = buyPrices[activeCoinId] !== undefined ? buyPrices[activeCoinId] : Math.round(currentPriceKRW * 0.98);
  const currentQuantity = quantities[activeCoinId] !== undefined ? quantities[activeCoinId] : (activeCoinId === "samsung" ? 100 : activeCoinId === "hynix" ? 50 : 30);
  ...
  return (
    <PriceProvider>
      <Helmet>
        <title>{coin.name} ({coin.symbol}) 실시간 주가 및 AI 지표 분석 | 알고리즘 트레이더</title>
              ...
              <div className="flex-1 max-w-2xl relative w-full flex flex-col gap-3" ref={searchInputRef}>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="시장, 자산 검색 (예: 리플, 도지종목)"
          ...
          {/* Metrics Row */}
            ...
            <div className="card min-w-[280px] p-5 md:p-6 relative overflow-hidden flex flex-col justify-between">
              ...
              <div className="mt-3 flex justify-between text-[11px] font-mono font-semibold text-on-surface-variant">
                <span>저가: {formatKRW(String(cryptoData?.low24h || coin.volLow))}</span>
                <span>고가: {formatKRW(String(cryptoData?.high24h || coin.volHigh))}</span>
              </div>
            </div>
            <MetricCard
              isLoading={loadingCrypto}
              label="시가총액"
              value={formatKRWMacro(String(cryptoData?.marketCap || coin.marketCap))}
              unit=""
                  ...
                  <div className="space-y-5 flex flex-col justify-center">
                    <div>
                      ...
                      <div className="flex justify-between text-[11px] text-on-surface-variant mt-1.5 px-1 flex-wrap gap-2">
                        ...
                        <button 
                          type="button"
                          onClick={() => handleBuyPriceChange(Math.round(currentPriceKRW * 0.98))}
                  ...
                  <div>
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        >
                          {formatKRW(String(cryptoData?.ma200 || coin.targetPrice))}
                        </p>
                      </div>
                    </div>
                  </div>
