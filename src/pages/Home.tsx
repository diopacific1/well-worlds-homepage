    ...
    <header className="relative pt-[140px] md:pt-[180px] pb-32 md:pb-40 flex flex-col items-center justify-center text-center min-h-[92vh] overflow-hidden">
      ...
      >
        <div className="space-y-8 flex flex-col items-center">
          ...
          {/* Majestic Hero Title - Elegantly Staggered with Custom Glow Text Effect */}
          <motion.h1 
            initial={{ opacity: 0, y: 25 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 1, delay: 0.15, ease: [0.16, 1, 0.3, 1] }}
            className="text-[3rem] sm:text-6xl md:text-[5.5rem] lg:text-[6.8rem] font-display font-black tracking-tight text-on-surface leading-[1.05] break-keep"
          >
            우물 속 깊은 곳, <br className="hidden md:block" />
            나만의 <span className="relative inline-block mt-2 md:mt-0 px-2">
          ...
          >
            수면 아래 고요히 부유하는 사유의 공간.<br />
            당신의 일상과 생각, 그리고 성장의 파동을 온전히 담아내다.
          </motion.p>
        </div>

        {/* Dynamic, responsive elegant Call To Actions with advanced interactive elements */}
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
      ...
      {/* Highly immersive, elegant scroll indicator with water ripple animation */}
      <motion.div 
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
